...
class mktowsClient 
{
  ...
  public function __construct($accessKey, $secretKey, $soapEndPoint, $timezone)
  {
      ...
      if (!$ok) {
        if ($faultCode != null) {
          if (strpos($faultCode, 'Client')) {
            ...
            _marketo_soap_watchdog_log("Client-side error. Line $err_line on file $err_file. $err_message.", WATCHDOG_ERROR);
          } else if (strpos($faultCode, 'Server')) {
            // This is a server error.  Call Marketo support with details.
            _marketo_soap_watchdog_log("Server-side error. Line $err_line on file $err_file. $err_message.", WATCHDOG_ERROR);
          } else {
            // W3C spec has changed :)
            // But seriously, Call Marketo support with details.
            _marketo_soap_watchdog_log("W3C spec has changed. Line $err_line on file $err_file. $err_message.", WATCHDOG_ERROR);
          }
        } else {
          // Not a good place to be.
          _marketo_soap_watchdog_log("Not a good place to be. Call Marketo Support. Line $err_line on file $err_file. $err_message.", WATCHDOG_ERROR);
        }
      }
      $success = FALSE;
    }
  ...
  public function requestCampaign($campId, $leadId)
  {
    $retStat = false;
 ...
 * 
mail|Email
field_profile_firstname|FirstName
field_profile_lastname|LastName
field_profile_title|Title
field_profile_organization|Company
field_profile_mobilephone|MobilePhone
field_profile_officephone|Phone
field_profile_country|Country
field_profile_country_state|State
field_profile_industry|Industry
field_profile_products_interest|PersonPrimaryLeadInterest
field_profile_web_address|Website
field_profile_employees|NumberOfEmployees
 **/
